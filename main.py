...
* 코스피/코스닥 각각 시가총액 상위 약 99개(데이터 소스의 사실상 한계)를 병렬로 스캔한다.
...
SCAN_PERCENTAGE = 1.0            # 그중 시가총액 상위 몇 %만 실제로 스캔할지 (1.0 = 100%, 즉 전부)
# ↑ 100%로 설정된 이유: 종목 목록 API(stock.naver.com/api/domestic/market/stock/default)가
#   시장당 시가총액 상위 약 99개까지만 응답하고, 그 이상은 페이지를 아무리 넘겨도
#   빈 배열([])을 명확하게 반환한다 (버그가 아니라 이 API 자체의 설계상 한계로 보임 —
#   "시장 요약"용 엔드포인트라 "전 종목 목록"용이 아닌 것으로 추정됨). 그래서 이미 상한선인
#   99개가 사실상 전체이므로 100%로 둬도 무방하다. 나중에 전 종목(800개+/1500개+)을 위한
#   다른 데이터 소스를 찾게 되면, 그때는 이 값을 다시 0.5 등으로 낮춰 의미 있게 쓸 수 있다.
